...
Noeud* rechercheCreeNoeudArbre(Reseau* R, ArbreQuat** a, ArbreQuat* parent, double x, double y){
    ...
    if ((*a)->noeud == NULL){
        ...
        if (x < xc){
            return (y < yc) ? (rechercheCreeNoeudArbre(R, &((*a)->so), *a, x, y)) : (rechercheCreeNoeudArbre(R, &((*a)->no), *a, x, y));
        } else { 
            return (y < yc) ? (rechercheCreeNoeudArbre(R, &((*a)->se), *a, x, y)) : (rechercheCreeNoeudArbre(R, &((*a)->ne), *a, x, y));
        }
    }
